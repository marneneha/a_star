    ...
    # initialize empty lists
        ...
        while True:
            ...
            # to check if the coordinate value entered is greater than the canvas dimensions
                ...
                continue   
            # (a & b).all()     
            if((canvas[canvas.shape[0]-1-y][x][0] & [255, 0, 0]).all() or 0<=x<5 or 0<=y<5):
        ...
        while True:
            ...
            try:
                ...
                print(bcolors.WARNING+"Hey! currently we only accept angle in multiple of 30. Will you enter something og form 30*n where n belongs to integers"+bcolors.ENDC)
                continue
            if angle < 0:
                angle += 360
        ...
        while True:
            ...
            if(int(step_size)<1 and int(step_size)>10):
                print(bcolors.WARNING+"Invalid, enter value between 1 and 10"+bcolors.ENDC)
            else:
                break
